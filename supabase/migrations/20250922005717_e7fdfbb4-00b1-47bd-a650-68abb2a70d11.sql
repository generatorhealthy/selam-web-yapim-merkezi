...
-- Schedule sitemap generation to run daily at 3 AM
...
-- Also schedule to run when new content is published (trigger on blog posts and specialists)
...
-- Create triggers for automatic sitemap generation
DROP TRIGGER IF EXISTS trigger_sitemap_on_blog_publish ON blogs;
CREATE TRIGGER trigger_sitemap_on_blog_publish
  AFTER INSERT OR UPDATE OF status ON blogs
  FOR EACH ROW
  WHEN (NEW.status = 'published')
  EXECUTE FUNCTION trigger_sitemap_generation();

DROP TRIGGER IF EXISTS trigger_sitemap_on_blog_post_publish ON blog_posts;
CREATE TRIGGER trigger_sitemap_on_blog_post_publish
  AFTER INSERT OR UPDATE OF status ON blog_posts
  FOR EACH ROW
  WHEN (NEW.status = 'published')
  EXECUTE FUNCTION trigger_sitemap_generation();

DROP TRIGGER IF EXISTS trigger_sitemap_on_specialist_add ON specialists;
CREATE TRIGGER trigger_sitemap_on_specialist_add
  AFTER INSERT OR UPDATE OF is_active ON specialists
  FOR EACH ROW
