-- Sipariş onaylandığında sözleşme içeriklerini otomatik oluşturan trigger
...
<p><strong>Sözleşme Oluşturulma Tarihi:</strong> ' || TO_CHAR(NEW.created_at, 'DD.MM.YYYY') || '</p>
<p><strong>Dijital Onaylama Tarihi:</strong> ' || TO_CHAR(NOW(), 'DD.MM.YYYY HH24:MI') || '</p>
...
Sözleşme Tarihi: ' || TO_CHAR(NEW.created_at, 'DD.MM.YYYY') || '
...
DROP TRIGGER IF EXISTS trigger_generate_contract_content ON public.orders;

CREATE TRIGGER trigger_generate_contract_content
  BEFORE UPDATE ON public.orders
  FOR EACH ROW
  EXECUTE FUNCTION public.generate_contract_content_on_approval();