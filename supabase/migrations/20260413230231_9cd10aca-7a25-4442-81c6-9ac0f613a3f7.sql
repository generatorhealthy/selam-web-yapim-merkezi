...
BEGIN
  ...
  SELECT 
    ...
    s.hospital::text,
    s.university::text,
    s.certifications::text,
    s.online_consultation::boolean,
    s.face_to_face_consultation::boolean,
    s.address::text,
    s.phone::text,
    s.email::text,
    s.working_hours_start::text,
    s.working_hours_end::text,
    s.available_days::text[],
    s.available_time_slots::jsonb,
    s.faq::text,
    s.seo_title::text,
    s.seo_description::text,
    s.seo_keywords::text,
    s.user_id::uuid,
    s.slug::text
...
GRANT EXECUTE ON FUNCTION public.get_public_specialist_by_slug(text) TO anon, authenticated;
