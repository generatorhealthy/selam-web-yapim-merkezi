-- Update the create_specialist_tests function to add default questions
...
AS $function$
BEGIN
  -- Eğer uzmanın uzmanlık alanı Psikolog ise, Psikolog testlerini ve sorularını ekle
  IF NEW.specialty = 'Psikolog' THEN
    -- Insert tests for Psikolog specialty
    ...
    -- Insert default questions for the newly created Psikolog tests
  ...
  -- Eğer uzmanın uzmanlık alanı Aile Danışmanı ise, Aile Danışmanı testlerini ve sorularını ekle
  ELSIF NEW.specialty = 'Aile Danışmanı' THEN
    -- Insert tests for Aile Danışmanı specialty
    INSERT INTO public.tests (title, description, category, specialty_area, status, is_active, specialist_id)
    SELECT 
      title || ' - ' || NEW.name,
      description,
    ...
    -- Insert default questions for the newly created Aile Danışmanı tests
    INSERT INTO public.test_questions (test_id, question_text, question_type, options, step_number, is_required) 
    SELECT 
      t.id,
      q.question_text,
  ...
  -- Eğer uzmanın uzmanlık alanı Psikolojik Danışmanlık ise, Psikolojik Danışmanlık testlerini ve sorularını ekle
  ELSIF NEW.specialty = 'Psikolojik Danışmanlık' THEN
    -- Insert tests for Psikolojik Danışmanlık specialty
    INSERT INTO public.tests (title, description, category, specialty_area, status, is_active, specialist_id)
    SELECT 
      title || ' - ' || NEW.name,
      description,
    ...
    -- Insert default questions for the newly created Psikolojik Danışmanlık tests
    INSERT INTO public.test_questions (test_id, question_text, question_type, options, step_number, is_required) 
    SELECT 
      t.id,
      q.question_text,
