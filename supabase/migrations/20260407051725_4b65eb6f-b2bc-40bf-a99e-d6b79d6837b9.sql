...
AS $function$
BEGIN
  IF NEW.status IN ('approved', 'completed') AND COALESCE(OLD.status, '') NOT IN ('approved', 'completed') THEN
    IF COALESCE(NEW.subscription_month, 1) = 1 OR NEW.is_first_order = true THEN
      IF NOT EXISTS (
        ...
        INSERT INTO public.automatic_orders (
          ...
          customer_address, customer_city, customer_type, company_name,
          company_tax_no, company_tax_office, package_name, package_type,
          amount, payment_method, monthly_payment_day, registration_date,
          total_months, current_month, paid_months, is_active
        ) VALUES (
          NEW.customer_name, NEW.customer_email, NEW.customer_phone, NEW.customer_tc_no,
          NEW.customer_address, NEW.customer_city, COALESCE(NEW.customer_type, 'individual'),
          NEW.company_name, NEW.company_tax_no, NEW.company_tax_office,
          NEW.package_name, NEW.package_type, NEW.amount,
