-- Function to send contract emails when order is approved
...
BEGIN
  ...
  IF NEW.status IN ('approved', 'completed') 
    ...
    -- Call the edge function to send contract emails
...
-- Create trigger for sending contract emails on order approval
DROP TRIGGER IF EXISTS trigger_send_contract_emails_on_approval ON public.orders;
CREATE TRIGGER trigger_send_contract_emails_on_approval
