...
// Strip Turkish/diacritic characters so the message stays inside the GSM-7 alphabet.
...
serve(async (req) => {
  ...
  try {
    ...
    // Mark successfully queued leads with welcome_sent_at so a subsequent run with skipSent=1 skips them.
    if (sentLeadIds.length > 0) {
      const nowIso = new Date().toISOString();
      // Update in chunks of 500 to keep the IN() list reasonable.
    ...
    console.error("send-reapply-bulk-sms error:", e);
